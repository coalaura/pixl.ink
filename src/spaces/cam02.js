...
const CAT02_TO_HPE_M = matmul3x3(XYZ_TO_LMS_HPE_MATRIX, CAT02_INV_MATRIX),
	HPE_TO_CAT02_MATRIX = matmul3x3(CAT02_MATRIX, LMS_TO_XYZ_HPE_MATRIX);
...
function makeParams(provided) {
	...
	const rgbW = matmul(alloc3(), CAT02_MATRIX, wpX, wpY, wpZ);

	const dR = 1 + d * (wpY / rgbW[0] - 1),
		dG = 1 + d * (wpY / rgbW[1] - 1),
		dB = 1 + d * (wpY / rgbW[2] - 1);

	const D_CAT02 = new Float64Array([dR, dG, dB]),
		D_CAT02_INV = new Float64Array([1 / dR, 1 / dG, 1 / dB]);
	...
	const rgbPW = matmul(alloc3(), CAT02_TO_HPE_M, rgbCW[0], rgbCW[1], rgbCW[2]),
		rgbAW = camAdapt(alloc3(), rgbPW[0], rgbPW[1], rgbPW[2], fl);

	const Aw = nbb * (2 * rgbAW[0] + rgbAW[1] + 0.05 * rgbAW[2]);
...
export default {
	name: "CAM02 JMh",
	long: "CAM02 (Color Appearance Model 2002), J-M-h",
	css: "cam02-jmh",
	tags: ["appearance_model"],
	base: "CIE 1931 XYZ",
	ui: {
		j: { from: 0, to: 100, step: 1, round: 0, name: "Lightness J", primary: true },
		m: { from: 0, to: 120, step: 1, round: 0, name: "Colorfulness M" },
	...
	from: (jmh, out = {}, params = defaults) => {
		...
		if (M < EPS_PRECISION || Jroot < EPS_PRECISION) {
			...
			matmul(v3, HPE_TO_CAT02_MATRIX, v3[0], v3[1], v3[2]);
			matmul(v3, CAT02_INV_MATRIX, v3[0] * D_INV[0], v3[1] * D_INV[1], v3[2] * D_INV[2]);
		...
		const alpha = M / (params.FL_ROOT * Jroot),
		...
		const denominator = 23 * p1Term + t * (11 * cosh + 108 * sinh);
		...
		matmul(v3b, HPE_TO_CAT02_MATRIX, v3b[0], v3b[1], v3b[2]);
		matmul(v3b, CAT02_INV_MATRIX, v3b[0] * D_INV[0], v3b[1] * D_INV[1], v3b[2] * D_INV[2]);
	...
	to: (xyz, out = {}, unclamped = false, params = defaults) => {
		...
		const D = params.D_CAT02,
			A_W = params.A_W,
			FL = params.FL;

		const v3 = matmul(alloc3(), CAT02_MATRIX, xyz.x * 100, xyz.y * 100, xyz.z * 100);

		matmul(v3, CAT02_TO_HPE_M, v3[0] * D[0], v3[1] * D[1], v3[2] * D[2]);

		camAdapt(v3, v3[0], v3[1], v3[2], FL);
		...
		const Cc = alpha * Math.sqrt(J / 100),
			M = Cc * params.FL_ROOT,
			h = hRad * RAD2DEG;
		...
		out.m = clamp(isAchromatic ? 0 : M / 120, 0, 1, unclamped);
	...
	expected: {
		...
		"#FFFFFF": { j: 1.0, m: 0.0191, h: 0.5854 },
		"#FF0000": { j: 0.4693, m: 0.6703, h: 0.0893 },
		"#00FF00": { j: 0.7975, m: 0.6387, h: 0.3796 },
		"#0000FF": { j: 0.2107, m: 0.5453, h: 0.7164 },
		"#FFFF00": { j: 0.9568, m: 0.4855, h: 0.2949 },
		"#00FFFF": { j: 0.8449, m: 0.3569, h: 0.55 },
		"#FF00FF": { j: 0.5407, m: 0.5895, h: 0.9165 },
		"#808080": { j: 0.4304, m: 0.0125, h: 0.5854 },
		"#FFA500": { j: 0.6893, m: 0.4052, h: 0.201 },
	},
};
