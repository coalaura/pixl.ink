...
vec3 rlab_to_xyz(vec3 rlab) {
    ...
    float y_r = L_R * 0.01;
    float x_r = spow(a_R / 430.0 + y_r, u_params.sigma);
    float z_r = spow(y_r - b_R / 170.0, u_params.sigma);
    float y_comp = spow(y_r, u_params.sigma);
...
vec3 xyz_to_rlab(vec3 xyz) {
    ...
    float x_r = spow(v3.x, u_params.sigmaInv);
    float y_r = spow(v3.y, u_params.sigmaInv);
    float z_r = spow(v3.z, u_params.sigmaInv);
