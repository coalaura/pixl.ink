...
- **sRGB**
  ...
  - Library: inputs/outputs are `[0,1]` as well (you don't pass 0-255; UI sliders map 0-255 to 0-1 for the viewer).

The **`ui` ranges** are for a UI built on top (like https://pixl.ink/) and do _not_ change the math. They indicate how to present sliders, not what `from`/`to` accept.
...
```js
import { cvd } from "color-encyclopedia";
...
```js
import { isColorPerceivable, SPECTRAL_LOCUS } from "color-encyclopedia";
...
```js
import { getForegroundColor } from "color-encyclopedia";
...
```js
import { whites } from "color-encyclopedia";
...
For UI or documentation tooling, the library exposes some classification helpers:

```js
import { tags, symbols } from "color-encyclopedia";
...
// "𝑙", "ℎ", "𝐶𝑧" - used in the pixl.ink UI for axis labels, legends, etc.
```

Each space's `tags` and `base` fields are purely descriptive; they don't change behavior but are very handy for building filtered lists or grouped documentation.
...
- **XYZ anchor**: Everything goes through D65/2° CIE XYZ. Spaces with other whites (Rec.601, NTSC, ProPhoto, DCI-P3, etc.) use Bradford or CAT02/CAT16 adaptation internally.
- **Pooled allocation**: Math routines use `alloc3`/`free3` pools for scratch `Float64Array`s to reduce GC pressure in interactive UIs.
